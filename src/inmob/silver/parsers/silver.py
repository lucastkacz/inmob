...
from inmob.silver.contracts import (
    CanonicalListing,
    CommercialTerms,
    FeatureSet,
    Location,
...
def _parse_zonaprop(metadata: RawArtifactMetadata, payload: bytes) -> CanonicalListing:
    ...
    if stripped.startswith("{"):
        try:
            data = _json_payload(payload)
        except (json.JSONDecodeError, ParserError):
            data = {}
        if data:
            return _parse_zonaprop_api(metadata, data)
...
def _parse_zonaprop_api(
    metadata: RawArtifactMetadata,
    data: dict[str, Any],
) -> CanonicalListing:
    posting_location = _as_dict(data.get("postingLocation"))
    address = _as_dict(posting_location.get("address"))
    geolocation = _as_dict(_as_dict(posting_location.get("postingGeolocation")).get("geolocation"))
    main_features = _as_dict(data.get("mainFeatures"))
    general_features = _as_dict(data.get("generalFeatures"))
    flags_features = _as_dict(data.get("flagsFeatures"))
    highlighted_features = _as_dict(data.get("highlightedFeatures"))
    price_amount, currency, operation_type = _zonaprop_api_price(data.get("priceOperationTypes"))
    expenses_amount, expenses_currency = _zonaprop_api_expenses(data.get("expenses"))
    real_estate_type = _as_dict(data.get("realEstateType"))
    publisher = _as_dict(data.get("publisher"))
    location_labels = _zonaprop_api_location_labels(posting_location)

    return _base(
        metadata,
        parser_id="zonaprop_json_v1",
        source_listing_id=_str(data.get("postingId") or data.get("postingCode")),
        canonical_url=metadata.target_metadata.get("public_url") or metadata.final_uri,
        title=_str(data.get("title") or data.get("generatedTitle")),
        commercial=CommercialTerms(
            price_amount=price_amount,
            currency=currency,
        ...
        surface=Surface(
            total_m2=_feature_num(main_features, "CFT100"),
            covered_m2=_feature_num(main_features, "CFT101"),
            uncovered_m2=_feature_num(main_features, "CFT102"),
            semicovered_m2=_feature_num(general_features, "Superficie Semicubierta (m²)"),
            terrace_m2=_feature_num(general_features, "Terraza"),
        ...
        location=Location(
            address=_str(address.get("name") or data.get("address")),
            neighborhood=_str(
                posting_location.get("neighborhood")
                or posting_location.get("zone")
                or (location_labels[0] if location_labels else None)
            ),
            city=_str(
                posting_location.get("city")
                or (location_labels[1] if len(location_labels) > 1 else None)
            ),
            province=_str(
                posting_location.get("province")
                or (location_labels[-1] if location_labels else None)
            ),
            latitude=_num(geolocation.get("latitude")),
            longitude=_num(geolocation.get("longitude")),
        ),
        seller=SellerContact(
            agency_name=_str(publisher.get("name")),
            agency_license=_str(publisher.get("license")),
            phone=_str(publisher.get("mainPhone") or publisher.get("partialPhone")),
            contact_url=_str(publisher.get("url")),
        ),
        published_at=_dt(data.get("createdAt") or data.get("createdDate")),
        publication_text=_str(data.get("descriptionNormalized") or data.get("description")),
        features=FeatureSet(
            rooms=_feature_int(main_features, "CFT1"),
            bedrooms=_feature_int(main_features, "CFT2"),
            bathrooms=_feature_int(main_features, "CFT3"),
            toilettes=_feature_int(main_features, "CFT7"),
            parking_spaces=_feature_int(main_features, "CFT4"),
            age_years=_feature_int(main_features, "CFT5"),
            property_type=_str(real_estate_type.get("name")),
            ...
            booleans={
                **_zonaprop_booleans(general_features),
                **_zonaprop_booleans(flags_features),
                **_known_booleans(data, "hasVideos", "hasTour", "hasPlans", "reserved"),
            },
            attributes={
                **_zonaprop_attributes(general_features),
                **_zonaprop_attributes(main_features),
                **_feature_attributes(flags_features, highlighted_features),
            },
            raw_features={
                "mainFeatures": main_features,
                "generalFeatures": general_features,
                "flagsFeatures": flags_features,
                "highlightedFeatures": highlighted_features,
            },
        ),
        source_specific={
            "posting_code": _str(data.get("postingCode")),
            "alphanumeric_key": _str(data.get("alphanumericKey")),
            "publisher_id": _str(publisher.get("publisherId") or publisher.get("id")),
            "real_estate_type_id": _str(real_estate_type.get("id")),
            "posting_type": _str(data.get("postingType")),
            "status": _str(data.get("status")),
            "modified_at": _str(data.get("modifiedAt") or data.get("modifiedDate")),
        },
    )
...
def _zonaprop_api_price(prices_data: Any) -> tuple[float | None, str | None, str | None]:
    if not isinstance(prices_data, list):
        return None, None, None
    for operation in prices_data:
        if not isinstance(operation, dict):
            continue
        operation_type = _str(_as_dict(operation.get("operationType")).get("name"))
        prices = operation.get("prices")
        if not isinstance(prices, list) or not prices:
            continue
        first = prices[0]
        ...
        if isinstance(currency, dict):
            ...
            currency_value = _str(currency or first.get("isoCode"))
        return _num(first.get("amount") or first.get("value")), currency_value, operation_type
    return None, None, None


def _zonaprop_api_expenses(expenses_data: Any) -> tuple[float | None, str | None]:
    expenses = _as_dict(expenses_data)
    currency = expenses.get("currency")
    if isinstance(currency, dict):
        currency_value = _str(
            currency.get("isoCode")
            or currency.get("code")
        ...
        currency_value = _str(currency or expenses.get("isoCode"))
    return _num(expenses.get("amount") or expenses.get("value")), currency_value


def _zonaprop_api_location_labels(location_data: dict[str, Any]) -> list[str]:
    labels: list[str] = []
    seen: set[str] = set()
    ...
    def visit(value: Any) -> None:
        if not isinstance(value, dict):
            return
        label = _str(value.get("name") or value.get("label"))
    ...
    for key in ("location", "subZone", "zone", "city", "province"):
        visit(location_data.get(key))
    return labels
