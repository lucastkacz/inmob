...
EXPORT_ROOT = Path("TOMAS_ACA_TENES_LOS_RAW_DE_MUDAFY")
PAGES = (1,)
PAGE_SIZE = 50
...
def test_mudafy_bulk_caba_buy_first_page_raw_export(tmp_path) -> None:
    ...
    search_targets = MudafySource.search_targets(criteria=criteria, pages=PAGES)
    context = IngestionRunContext(run_id="TOMAS_ACA_TENES_LOS_RAW_DE_MUDAFY")
    ...
    with MudafySource(targets=search_targets, traffic_controller=traffic) as search_source:
        for search_request in search_source.plan_requests(context):
            ...
            assert len(page_listing_targets) >= 40
    ...
    assert discovered_listing_count >= 40
    ...
    with MudafySource(targets=listing_targets, traffic_controller=traffic) as listing_source:
...
def _looks_like_listing_html(
    *,
    ...
    if len(payload) < 50_000:
        return False
    if b"application/ld+json" not in payload:
        return False
