...
EXPORT_ROOT = Path("TOMAS_ACA_TENES_LOS_RAW_DE_ZONAPROP")
PAGES = (1,)
PAGE_SIZE = 30
...
def test_zonaprop_bulk_caba_buy_first_page_raw_export(tmp_path) -> None:
    ...
    search_targets = ZonapropSource.search_targets(criteria=criteria, pages=PAGES)
    context = IngestionRunContext(run_id="TOMAS_ACA_TENES_LOS_RAW_DE_ZONAPROP")
    ...
    with ZonapropSource(targets=search_targets, traffic_controller=traffic) as search_source:
    ...
    with ZonapropSource(targets=listing_targets, traffic_controller=traffic) as listing_source:
