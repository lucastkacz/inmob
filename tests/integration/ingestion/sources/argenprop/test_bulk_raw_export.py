...
EXPORT_ROOT = Path("TOMAS_ACA_TENES_LOS_RAW_DE_ARGENPROP")
PAGES = (1,)
PAGE_SIZE = 20
...
def test_argenprop_bulk_capital_federal_newest_first_page_raw_export(tmp_path) -> None:
    criteria = ArgenpropSearchCriteria(
        ...
        page_size=PAGE_SIZE,
        sort="masnuevos",
        label="capital-federal-apartment-sale-newest",
    )
    ...
    with ArgenpropSource(targets=search_targets, traffic_controller=traffic) as search_source:
        for search_request in search_source.plan_requests(context):
            ...
            assert "orden-masnuevos" in search_target.uri
    ...
    with ArgenpropSource(targets=listing_targets, traffic_controller=traffic) as listing_source:
